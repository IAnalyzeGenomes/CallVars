...
rule all:
    input:
        ...
        Report3 = expand(
            "CallVars/NoDupReads/{sample}_PerBaseCov.txt", sample=config["SAMPLE"]),
        Report4 = expand(
            "CallVars/NoDupReads/{sample}_PerBaseCov_LessThan20.txt", sample=config["SAMPLE"])
...
include: "rules/HaplotypeCaller.py"
